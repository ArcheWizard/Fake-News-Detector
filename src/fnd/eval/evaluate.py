...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate a saved model on the test split"
    )
    parser.add_argument("--config", help="Path to YAML config file (optional)")
    parser.add_argument(
        "--model_dir",
        required=True,
        help="Directory containing saved model and tokenizer",
    )
    parser.add_argument(
        "--out_dir", required=True, help="Directory to save evaluation artifacts"
    )
    ...
    parser.add_argument(
        "--max_seq_length", type=int, help="Override max sequence length"
    )
    ...
    if args.config:
        overrides = {
            ...
            if v is not None and k not in ("config", "model_dir", "out_dir")
        }
    ...
    trainer = Trainer(
        model=model, data_collator=data_collator, compute_metrics=compute_metrics
    )
    ...
    if config.eval.save_plots:
        ...
        disp = ConfusionMatrixDisplay(
            confusion_matrix=cm, display_labels=["real", "fake"]
        )
